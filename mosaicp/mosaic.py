...
class MosaicImage:
    ...
    def add_tile(self, tile, coords):
        """Adds the provided image onto the mosaic at the provided coordinates."""
        try:
...
def coords_from_middle(x_count, y_count, y_bias=1, shuffle_first=0, ):
    ...
    And we don't get "lines" where the same-best images
    get used at the start.

    y_bias - if we are using non-square coordinates, we can
        influence the order to be closer to the real middle.
        If width is 2x height, y_bias should be 2.

    shuffle_first - We can shuffle the first X coords
        so that we don't use all the same-best images
        in the same spot -  in the middle

    from movies.mosaic_mem import coordinates_from_middle
    x = 10
    y = 10
